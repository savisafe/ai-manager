...
@Controller("admin/prompt-profiles")
@UseGuards(AdminJwtAuthGuard, AdminRolesGuard)
@AdminRoles(AdminRole.ADMIN, AdminRole.MANAGER)
export class AdminPromptProfilesController {
  constructor(
  ...
  @Get()
  async list() {
    return this.prisma.promptProfile.findMany({ orderBy: { updatedAt: "desc" } });
  }

  @Get(":id")
  async get(@Param("id") id: string) {
    const row = await this.prisma.promptProfile.findFirst({
      where: { OR: [{ id }, { slug: id }] },
    });
    if (!row) {
  ...
  async create(@Body() body: unknown) {
    ...
    const row = await this.prisma.promptProfile.create({
      data: {
  ...
  async patch(@Param("id") id: string, @Body() body: unknown) {
    ...
    const existing = await this.prisma.promptProfile.findUnique({ where: { id } });
    if (!existing) {
      throw new NotFoundException();
    }
    const row = await this.prisma.promptProfile.update({
      where: { id },
      data: {
        slug: parsed.data.slug,
  ...
  async remove(@Param("id") id: string) {
    await this.prisma.promptProfile.delete({ where: { id } }).catch(() => {
      throw new NotFoundException();
    });
    this.configManagement.invalidateAll();
    return { ok: true };
